 ...
 */
export function ReduxClassWrapper<T>(reducer: Reducer<MixedState<T>>): Reducer<MixedState<T>> {
  ...
  function decoratedStateReducer(state: MixedState<T> | undefined, action: TAction): MixedState<T> {
    const newState = reducer(state, action)
    return stateReducerHandler(newState)
  }

  function createReducer(state: MixedState<T> | undefined, action: TAction): MixedState<T> {
    const newState = reducer(state, action)
  ...
  function main(state: MixedState<T> | undefined, action: TAction): MixedState<T> {
