...
export function bindPrototype(prototype, key, _target, needsNew = false) {
...
export function bindMethods(_target, _source, keys, needsNew = false, method = bindPrototype, exclude = []) {
  for (let key of keys) {
    if (_source.prototype[key]) {
      if (exclude.indexOf(key) === -1) {
        let desc = Object.getOwnPropertyDescriptor(_source.prototype, key)
        Object.defineProperty(_target.prototype, key, { ...desc,
